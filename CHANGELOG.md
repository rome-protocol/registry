...
- **`services/rome-ui-worker/service.json#servesPrograms[0].until`** ([#30](https://github.com/rome-protocol/registry/pull/30)) — initially set to `2026-05-02T21:23:53Z` (DP1d program-close time) when the program was closed via `solana program close` (sig `DVNMTvXdjxoktqcMFiVwnd5yP6Rr2Ub1VHq47ezTi67AU474WzMsGbdNoaSK6fMkov6qvAgXPVpqr8c6SnLcoPA`, slot 459658302; 12.476 SOL reclaimed to the cold-ledger upgrade authority `Dcbhma9t1e3KrNczC5aHKZJBYjyBWb5TFmsVR9r8uqoj`).
- **`services/rome-ui-worker/service.json`** ([#31](https://github.com/rome-protocol/registry/pull/31)) — corrected the `until` semantics. The schema description for `servesPrograms[].until` is *"no longer served"*, not *"program closed"* — the worker stopped serving DP1d when it died, 22 min before the program-close itself. Both `servesPrograms[].until` (DP1d + 5uBA) set to `2026-05-02T21:01:17Z` (the rome-ops#198 merge time = worker VM destroyed); `lifecycle.decommissionedAt` set to the same. The 5uBA `until` is independent of Sattvik's pending program close — the worker stopped serving 5uBA at the same moment regardless of program lifetime.
- **`services/rome-ui-worker/service.json`** (this entry) — clear residual drift in fields that still pointed at destroyed infrastructure: drop `deployment.location` (referenced the destroyed `gcp-vm://…/devnet-rome-ui` VM), zero `configRefs` (`ansible/inventories/devnet-rome-ui-gcp/group_vars/host_rome_ui.yml` was deleted in rome-ops#198; `rome-ui:deploy/chains.sample.yaml` is downstream cleanup), zero `secretRefs` (the `gcp:projects/rome-developers/secrets/rome-ui-settle-payers` secret returns `NOT_FOUND` in `gcloud secrets describe` and history search shows the entry only ever existed in the registry — speculative metadata never realized in GCP). The service entry is preserved (not deleted) as audit history per the schema's reference-counted lifecycle model; Phase 5 worker re-provisioning will re-populate `deployment.location` / `configRefs` / `secretRefs` against the new RomeD-prefixed program.

After this entry: the rome-ui-worker entry shows `decommissionedAt` set, both `servesPrograms[]` entries with `until` populated, and no fields pointing at non-existent infrastructure. The `Phase 4 — registry verification gate` per `project_clean_slate_execution_plan.md` is effectively passed on the registry side; only Phase 0 (Squads V4 multisig setup with Sattvik) gates Phase 5.

- **`package.json` / `package-lock.json`** — `0.4.13` → `0.4.14` (data-only patch bump per `docs/SCHEMA_VERSIONING.md`; analogous to the `Status flip ↔ retired` patch-bucket policy — service-state corrections are content updates, no schema change).
...
- **`chains/121215-maximus/`** — removed (9 files: chain, contracts, tokens, bridge, oracle, endpoints, gasPricing, operationalLimits, NOTES). Retired 2026-05-01 (CHANGELOG entry `### Changed — 121215-maximus status: live → retired`).
- **`chains/121222-subura/`** — removed (9 files). Retired 2026-05-01 (CHANGELOG entry `### Changed — 121222-subura status: live → retired`).
- **`chains/121225-esquiline/`** — removed (9 files). Retired 2026-05-01 (CHANGELOG entry `### Changed — 121225-esquiline status: live → retired`).
- **`chains/121228-cassius/`** — removed (8 files; no gasPricing.json was ever populated). Retired 2026-05-01.
- **`chains/121299-aventine/`** — removed (8 files; no oracle.json — Aventine never wired oracle adapters). Retired 2026-05-01 (CHANGELOG entry `### Changed — 121299-aventine status: live → retired`).
- **`README.md`** — `## Available chains` table reduced to Marcus only; appended a one-line note pointing readers at this CHANGELOG entry and the v0.4.x CDN tags for archival reads.
- **`docs/REGISTRATION_ARCHITECTURE.md`** — `deployment_mode: internal` example updated from `(testrollup, subura, maximus, marcus, esquiline)` to `(Marcus and the local testrollup stack)`.
- **`tools/liveness.ts`** — header comment dropped the `subura/esquiline/maximus during the v0.2 data sweep` reference; the retired-chain skip logic is unchanged (still a no-op now since no retired entries exist in `chains/`).
- **`tools/add-bundle.ts`** — two illustrative comments rewritten: `humanizeName` example slug `cassius-test` → `marcus[-test]`, and the gas-token sentinel-address comment cited example chain `(marcus, esquiline)` → `(matches marcus)`.
- **`package.json` / `package-lock.json`** — `0.4.10` → `0.4.11` (data-only patch bump per `docs/SCHEMA_VERSIONING.md`; no schema changes; the `Status flip ↔ retired` line of the patch-bucket policy is the closest classification — directory removal is a follow-on of the `live → retired` flip already done in prior patches). Lockfile bumped via surgical `jq` edit per the cross-platform pitfall noted under v0.4.10.

### Why
Every chain except Marcus was retired in 2026-04 → 2026-05 as the active set contracted. Carrying their directories in `chains/` made the registry surface look noisier than it is — a fresh consumer browsing GitHub would see five retired-status directories alongside the one live one. The retirement history is fully preserved in CHANGELOG.md and via immutable v0.4.x CDN tags; the directories themselves were the only place `status: retired` data lived, and `tools/liveness.ts` already skipped them in CI. Removing the directories collapses the visible chain set to "what works today."
