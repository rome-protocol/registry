...
beforeEach(() => {
  ...
  const chainDir = path.join(root, "chains/121226-marcus");
  mkdirSync(chainDir, { recursive: true });
  writeFileSync(path.join(chainDir, "chain.json"), JSON.stringify({
    chainId: 121226, name: "Rome Marcus", network: "testnet",
...
describe("public NPM API", () => {
  it("getChain returns the seeded Marcus entry", () => {
    const m = getChain(121226);
    expect(m?.chainId).toBe(121226);
    expect(m?.name).toBe("Rome Marcus");
  });
